...
<?php }else{
    ...
    $films = $db->queryArray("SELECT * FROM film ORDER BY num_film DESC;");
        $count = $db->querySingle("SELECT COUNT(*) AS nb FROM film;");
    echo "<h3>Il y a $count->nb films dans la base de données </h3>";
	...
	<h3 style="cursor:pointer;" onclick="location.href='Films.php?num_film=<?php echo htmlspecialchars($film->num_film);?>'"><?php echo htmlspecialchars($film->titre); ?> </h3>
<table><tr><td> <img align="middle;" alt="affiche" name="affiche" src="<?php echo htmlspecialchars($film->affiche);?>"></td>
<td><?php if(!empty($_SESSION['login'])){
    $num_film = $film->num_film;
if((isset($_SESSION['login'])) AND ($_SESSION['login'] == 'admin')) {echo "<span style='float:right;' <i><a href='Modif_films.php?num_film=$num_film'> Modifier le film</a></i></span>"; }

if($_SESSION['login'] == 'admin'){ echo "<a href='Suppr_film.php?num_film=$num_film&ret=admin_films'>Supprimer le film</a>";}
				    } ?></td></tr></table>
   </div><?php  }?>
